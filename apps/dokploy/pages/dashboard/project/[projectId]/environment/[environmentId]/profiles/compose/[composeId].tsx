...
const normalizeComposeTab = (value: unknown): TabState => {
...
const Service = (
	...
	const [tab, setTab] = useState<TabState>(normalizeComposeTab(activeTab));

	useEffect(() => {
		if (router.query.tab) {
			setTab(normalizeComposeTab(router.query.tab));
		}
	}, [router.query.tab]);
							...
							{data?.server?.serverStatus === "inactive" ? (
								...
								<Tabs
									...
									onValueChange={(e) => {
										...
										const newPath = `/dashboard/project/${projectId}/environment/${environmentId}/profiles/compose/${composeId}?tab=${e}`;
										router.push(newPath);
									}}
								>
