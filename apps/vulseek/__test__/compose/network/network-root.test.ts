...
import { addSuffixToNetworksRoot, generateRandomHash } from "@vulseek/server";
...
test("It shoudn't add suffix to vulseek-network", () => {
	...
	expect(networks).toBeDefined();
	for (const networkKey of Object.keys(networks)) {
		expect(networkKey).toContain("vulseek-network");
	}
});
