import type { ComposeSpecification } from "@vulseek/server";
import {
...
test("It shoudn't add suffix to vulseek-network in services", () => {
	...
	expect(service?.networks).toContain("vulseek-network");
});
...
test("It shoudn't add suffix to vulseek-network in services multiples cases", () => {
	...
	expect(redis?.networks).toHaveProperty("vulseek-network");
	expect(dbNetworks["vulseek-network"]).toBeDefined();
	expect(apiNetworks["vulseek-network"]).toBeDefined();
});
