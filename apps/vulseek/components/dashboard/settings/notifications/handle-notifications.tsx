...
const notificationBaseSchema = z.object({
	...
	vulseekRestart: z.boolean().default(false),
		...
		if (notification) {
			if (notification.notificationType === "slack") {
				form.reset({
					...
					vulseekRestart: notification.vulseekRestart,
								...
								{!isCloud && (
									<FormField
										...
										render={({ field }) => (
											<FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm gap-2">
												<div className="space-y-0.5">
													<FormLabel>Vulseek Restart</FormLabel>
													<FormDescription>
														Trigger the action when vulseek is restarted.
													</FormDescription>
												</div>
												<FormControl>
													<Switch
