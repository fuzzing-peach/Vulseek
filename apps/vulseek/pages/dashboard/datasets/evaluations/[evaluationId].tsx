...
const EvaluationDetailPage = () => {
	...
	const query = api.dataset.evaluations.one.useQuery(
		{ evaluationId },
		{
			...
			refetchInterval: (data) => data && ["running", "pending", "paused", "finalizing"].includes(data.status) ? 3000 : false,
		},
	);
	...
	const active = ["running", "pending", "paused", "finalizing"].includes(query.data.status);
	...
	return (
		<>
			...
			<div className="w-full space-y-5">
				...
				<Card><CardHeader><CardTitle>Trials</CardTitle><CardDescription>Samples run in round-robin order. Each scan job retains its normal task, file, and session artifacts.</CardDescription></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-3">Sample</th><th className="p-3">Repetition</th><th className="p-3">Status</th><th className="p-3">Duration</th><th className="p-3">Tokens</th><th className="p-3">Cost</th><th className="p-3">Scan Job</th></tr></thead><tbody>{query.data.trials.map((trial) => <tr key={trial.trialId} className="border-b align-top"><td className="p-3"><div className="font-medium">{trial.sample?.title || trial.sample?.sampleKey || trial.sampleId}</div><div className="font-mono text-xs text-muted-foreground">{trial.sample?.sampleKey || trial.sampleId}</div></td><td className="p-3">{trial.repetition}</td><td className="p-3 capitalize">{trial.status}<div className="text-xs text-muted-foreground">post-scan: {trial.postScanStatus}</div></td><td className="p-3">{trial.durationMs ? `${Math.round(trial.durationMs / 1000)}s` : "-"}</td><td className="p-3">{numberFormat.format(trial.totalTokens)}</td><td className="p-3">{moneyFormat.format(trial.estimatedCost)}</td><td className="p-3">{trial.scanJobId ? <Link className="inline-flex items-center gap-1 font-mono text-xs text-primary hover:underline" href={`/dashboard/datasets/jobs/${encodeURIComponent(trial.scanJobId)}`}>{trial.scanJobId.slice(0, 12)}<ExternalLink className="size-3" /></Link> : "-"}{trial.errorMessage && <div className="mt-1 max-w-xs text-xs text-destructive">{trial.errorMessage}</div>}</td></tr>)}</tbody></table></div></CardContent></Card>
				{query.data.postEvaluationResult !== null && <Card><CardHeader><CardTitle>Post-evaluation result</CardTitle></CardHeader><CardContent><pre className="max-h-96 overflow-auto rounded-md bg-muted p-4 text-xs">{JSON.stringify(query.data.postEvaluationResult, null, 2)}</pre></CardContent></Card>}
				{query.data.errorMessage && <Card><CardHeader><CardTitle className="text-destructive">Error</CardTitle></CardHeader><CardContent className="text-sm text-destructive">{query.data.errorMessage}</CardContent></Card>}
			</div>
		</>
	);
};
