...
export const notificationRouter = createTRPCRouter({
	...
	testTelegramConnection: adminProcedure
		...
		.mutation(async ({ input }) => {
			try {
				await sendTelegramNotification(input, "Hi, From Vulseek 👋");
				return true;
			} catch (error) {
				throw new TRPCError({
					code: "BAD_REQUEST",
		...
		.mutation(async ({ input }) => {
			try {
				...
				await sendDiscordNotification(input, {
					...
					description: decorate(">", "Hi, From Vulseek 👋"),
					color: 0xf3f7f4,
				});
	...
	receiveNotification: publicProcedure
		.input(
			z.object({
				ServerType: z.enum(["Vulseek", "Remote"]).default("Vulseek"),
		...
		.mutation(async ({ input }) => {
			try {
				await sendNtfyNotification(
					...
					"Test Notification",
					"",
					"view, visit Vulseek on Github, https://github.com/fuzzing-peach/Vulseek, clear=true;",
					"Hi, From Vulseek 👋",
				);
				return true;
			} catch (error) {
