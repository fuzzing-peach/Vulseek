import { IS_CLOUD, removeScheduleJob, scheduleJob } from "@vulseek/server";
import { db } from "@vulseek/server/db";
import { deployments } from "@vulseek/server/db/schema/deployment";
import {
	createScheduleSchema,
	schedules,
	updateScheduleSchema,
} from "@vulseek/server/db/schema/schedule";
import { runCommand } from "@vulseek/server/index";
import {
	createSchedule,
	deleteSchedule,
	findScheduleById,
	updateSchedule,
} from "@vulseek/server/services/schedule";
	...
	list: protectedProcedure
		...
		.query(async ({ input }) => {
			const where = {
				...
				"vulseek-server": eq(schedules.userId, input.id),
			};
