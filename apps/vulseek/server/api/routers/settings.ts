...
import { generateOpenApiDocument } from "@vulseek/trpc-openapi";
...
export const settingsRouter = createTRPCRouter({
	reloadServer: adminProcedure.mutation(async () => {
		...
		await reloadDockerResource("vulseek");
		return true;
	}),
	cleanRedis: adminProcedure.mutation(async () => {
		...
		const { stdout: containerId } = await execAsync(
			`docker ps --filter "name=vulseek-redis" --filter "status=running" -q | head -n 1`,
		);
	...
	reloadRedis: adminProcedure.mutation(async () => {
		...
		await reloadDockerResource("vulseek-redis");
	...
	readWebServerTraefikConfig: adminProcedure.query(() => {
		...
		const traefikConfig = readConfig("vulseek");
		return traefikConfig;
	}),
	updateWebServerTraefikConfig: adminProcedure
		...
		.mutation(async ({ input }) => {
			...
			writeConfig("vulseek", input.traefikConfig);
			return true;
		}),
	...
	updateServer: adminProcedure.mutation(async () => {
		...
		// This causes restart of vulseek, thus it will not finish executing properly, so don't await it
	...
	getVulseekVersion: protectedProcedure.query(() => {
		return packageInfo.version;
	}),
	getReleaseTag: protectedProcedure.query(() => {
		return getVulseekImageTag();
	}),
	...
	getOpenApiDocument: protectedProcedure.query(
		async ({ ctx }): Promise<unknown> => {
			...
			openApiDocument.info = {
				title: "Vulseek API",
				description: "Endpoints for vulseek",
				version: "1.0.0",
			};
	...
	toggleRequests: adminProcedure
		...
		.mutation(async ({ input }) => {
			...
			if (input.enable) {
				const config = {
					accessLog: {
						filePath: "/etc/vulseek/traefik/dynamic/access.log",
						format: "json",
						bufferingSize: 100,
						filters: {
							retryAttempts: true,
	...
	getVulseekCloudIps: adminProcedure.query(async () => {
		if (!IS_CLOUD) {
			return [];
		}
		const ips = process.env.VULSEEK_CLOUD_IPS?.split(",");
		return ips;
	}),
});
