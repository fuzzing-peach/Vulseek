...
export const runDatasetEvaluation = async (evaluationId: string) => {
	...
	try {
		...
		await db.update(datasetEvaluations).set({ status: completedWithErrors ? "completed_with_errors" : "completed", finishedAt: new Date().toISOString(), updatedAt: new Date().toISOString() }).where(eq(datasetEvaluations.evaluationId, evaluationId));
	} catch (error) {
...
export const recoverPendingDatasetEvaluations = async () => {
	const rows = await db.select({ evaluationId: datasetEvaluations.evaluationId }).from(datasetEvaluations).where(inArray(datasetEvaluations.status, ["pending", "running"]));
