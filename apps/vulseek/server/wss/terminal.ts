...
cat /etc/vulseek/ssh/auto_generated-vulseek-local.pub >> $HOME/.ssh/authorized_keys && \\
echo "✓ Vulseek SSH key added successfully. Reopen the terminal in Vulseek to reconnect."
# ----------------------------------------`;

const COMMAND_TO_GRANT_PERMISSION_ACCESS = `
# ----------------------------------------
sudo chown -R $USER:$USER /etc/vulseek/ssh
# ----------------------------------------
`;
