...
Use GitHub Actions to build the release Docker image for Vulseek, publish it to GitHub Container Registry, and separate local development from release usage so local release builds are no longer required.
...
- Delete `apps/vulseek/docker/build.sh` during implementation.
- Delete `apps/vulseek/docker/push.sh` during implementation.
- Delete `apps/vulseek/docker/feat.sh` during implementation.
...
- `dev.sh`: keep this local Swarm-based dev workflow; it currently builds `vulseek-dev:latest` locally.
- `run.sh`: add this release workflow script; it should pull and run the GHCR release image, defaulting to the `latest` tag.
- `apps/vulseek/docker/build.sh`: obsolete Docker Hub-oriented multi-arch build helper; delete.
- `apps/vulseek/docker/push.sh`: obsolete Docker Hub-oriented multi-arch push helper; delete.
- `apps/vulseek/docker/feat.sh`: obsolete ad hoc helper; delete.
...
- Keep `Dockerfile.server`, `Dockerfile.schedule`, and `Dockerfile.monitoring`.
- Delete `apps/vulseek/docker/build.sh`, `apps/vulseek/docker/push.sh`, and `apps/vulseek/docker/feat.sh` because they are Docker Hub-specific or obsolete and should not remain alongside the new GHCR workflow.
...
7. Delete `apps/vulseek/docker/build.sh`, `apps/vulseek/docker/push.sh`, and `apps/vulseek/docker/feat.sh` during implementation, and remove any references to those obsolete Docker Hub-oriented helpers.
...
- `apps/vulseek/docker/build.sh`, `apps/vulseek/docker/push.sh`, and `apps/vulseek/docker/feat.sh` are gone.
